...
# image_path = '/Users/apple/Downloads/VisualNav/cvmlp-p4/demo_images/cat.jpg'
# image, image_tensor = image_from_path(image_path); # image.show()
# print("Image shape: {}".format(image_tensor.shape))
# image_array = np.asarray(image)/255

################################################ OBTAIN HEATMAP ################################################
def heatmap_from_image(tensor, model, layer, category=None):
    cam = GradCAM(model=model, target_layer=layer, use_cuda=False)
    grayscale_cam = cam(input_tensor=tensor, target_category=category)
    grayscale_cam = grayscale_cam[0, :]
    max_indices = np.where(grayscale_cam == grayscale_cam.max())
    max_x = max_indices[1][0]; max_y = max_indices[0][0]
    return grayscale_cam, [max_x, max_y]

# model = resnet50(pretrained=True); layer = model.layer4[-1]
# # grayscale_cam, [x, y] = heatmap_from_image(image_tensor, model, layer, 230)
# grayscale_cam, [x, y] = heatmap_from_image(image_tensor, model, layer, None)
...
# box = {'xmin': 63, 'ymin': 18, 'xmax': 374, 'ymax': 500}
